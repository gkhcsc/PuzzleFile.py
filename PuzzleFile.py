...
class Puzzle(QWidget):
    ...
    def contextMenu(self, point):
        ...
        # pix_other_Bronya.pyqtConfigure(triggered = self.pix_other_Bronya_replace)   #  一种连接槽的另一种方法

        以下为原本的代码，上面的代码均已没替换成 object = QAction("str",parent,targgered=***) 的形式,若要修改回来删除 targgered=*** 并将下面的代码解除注释即可
        # pix_other_SeeleVollerei.triggered.connect(
        #     self.pix_other_SeeleVollerei_replace)
        # pix_other_Theresa.triggered.connect(self.pix_other_Theresa_replace)

        注：下面的代码也多少会改成这种新形式，因为他简洁。
        '''
    ...
    def show_result(self):
        try:
            self.txt_line.setText(
                str(self._data._result[self.index_array_verify]))
